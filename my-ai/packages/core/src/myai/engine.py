...
def repo_root() -> Path:
    env = os.environ.get("MYAI_ROOT")
    if env:
        return Path(env)
    here = Path(__file__).resolve()
    candidates = [here.parent, *here.parents, Path.cwd(), Path.cwd() / "my-ai"]
    for parent in candidates:
        if (parent / "data/registry/products.json").exists():
            return parent
    # Last resort: source tree when running from packages/core/src/myai/engine.py
