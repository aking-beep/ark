...
test("landing, privacy, transparency, and scored sample", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: /find AI Fit/i })).toBeVisible();
  ...
  await expect(page.getByText(/AI Fit score/i)).toBeVisible({ timeout: 20_000 });
...
test("assessment intro gate", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: /find AI Fit/i })).toBeVisible();
